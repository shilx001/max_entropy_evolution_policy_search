...
class Policy:
    ...
    def forward_update(self, rollouts, baseline_reward, sigma_rewards):
        step = np.zeros(self.theta.shape)
        for r, delta in rollouts:
            step += (r - baseline_reward) * delta
...
class ARSTrainer:
    ...
    def train(self):
        ...
        for step in range(self.hp.nb_steps):
            ...
            #base_rewards = self.explore()
            # Compute the standard deviation of all rewards
            sigma_rewards = np.array(positive_rewards).std()

            # Sort the rollouts by the max(r_pos, r_neg) and select the deltas with best rewards
            rollouts = [(positive_rewards[k], deltas[k]) for k in range(len(deltas))]
            # print('Std in scores is: ',np.std(list(scores.values())))
            # Update the policy
            self.policy.forward_update(rollouts, 0, sigma_rewards)
