...
class ARSTrainer:
    ...
    def explore(self, direction=None, delta=None, print_done=False, with_noise=False):
    ...
    def train(self):
        ...
        for step in range(self.hp.nb_steps):
            ...
            for k in range(self.hp.num_deltas):
                positive_rewards[k] = self.explore(direction="+", delta=deltas[k], with_noise=True)
                negative_rewards[k] = self.explore(direction="-", delta=deltas[k], with_noise=True)
            ...
            # print('Std in scores is: ',np.std(list(scores.values())))
            # Update the policy
            ...
            reward_evaluation = self.explore(print_done=True, with_noise=False)
        ...
        return return_reward.copy(), return_std.copy()
...
for i in range(5):
    ...
    pickle.dump(reward, open(env + '_ars_analysis_seeds_' + str(seeds[i]), 'wb'))
